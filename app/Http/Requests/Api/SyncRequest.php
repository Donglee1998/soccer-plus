...
class SyncRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        // sync tactic
        $rules['*.tactics.*.id']         = ['required'];
        $rules['*.tactics.*.title']      = ['required', 'max_mb:255'];
        $rules['*.tactics.*.explain']    = ['required', 'max_mb:255'];
        $rules['*.tactics.*.type']       = ['required', 'in:' . implode(',', config('constants.tactic_type.key'))];
        $rules['*.tactics.*.status']     = ['required', 'in:' . implode(',', config('constants.tactic_status.key'))];
        $rules['*.tactics.*.pitch']      = ['required', 'in:' . implode(',', config('constants.tactic_pitch.key'))];
        $rules['*.tactics.*.background'] = ['required', 'numeric'];
